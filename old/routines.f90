...
#include "globalMacros.txt"
    ...
    real (kind=rk) :: temp(numPointsA, numAIME, numPointsSPA, numPointsY,numPointsL,numPointsA), tempEV(numPointsA, numAIME, numPointsSPA, numPointsY) !  tempA(numPointsA, numAIME, numPointsY,numPointsA)
    ...
    do typeSim = 1, numPointsType
        ...
        do ixt=Tperiods,1, -1                               ! Loop from time T-1 to 1
            ...
            !write (format_numpeepcols_int,*),'(',numPointsA*numAIME*numPointsSPA*numPointsY
            !format_numpeepcols_int = trim(format_numpeepcols_int) // 'E15.3)'            
            !
            !write (outFile, *), trim("..\\PolicyFuncs\polType"),typeSim,trim("Period"),ixt,".txt"
            !outfile=ADJUSTL(outfile)
            !temp =  modelObjects%policy(ixt,typeSim,:,:, :,:,:,:)
            !inquire (iolength=requiredl) temp
            !open (unit=201, file=outfile, status='unknown',recl=requiredl, action='write')
            !write (201,format_numpeepcols_int) temp
            !close( unit=201)
            !
            !!open (unit=201, file=outfile, status='unknown',recl=requiredl, action='read')
            !!read (201,*) temp
            !!close( unit=201)
            !!
            !!temp = temp - modelObjects%policy(ixt,typeSim,:,:, :,:,:,:) 
            !!write (*,*) maxval(temp)
            !
            !write (format_numpeepcols_int,*),'(',numPointsA
            !format_numpeepcols_int = trim(format_numpeepcols_int) // 'E15.3)'
            !write (outFile, *), trim("..\\PolicyFuncs\EVType"),typeSim,trim("Period"),ixt,".txt"
            !outfile=ADJUSTL(outfile)
            !tempEV =  modelObjects%EV(ixt,typeSim,:,:, :,:)
            !inquire (iolength=requiredl) tempEV
            !open (unit=201, file=outfile, status='unknown',recl=requiredl, action='write')
            !write (201,format_numpeepcols_int) tempEV
            !close( unit=201)
            !
            !!open (unit=201, file=outfile, status='unknown',recl=requiredl, action='read')
            !!read (201,*) tempEV
            !!close( unit=201)
            !!
            !!tempEV = tempEV - modelObjects%EV(ixt,typeSim,:,:, :,:) 
            !!write (*,*) maxval(temp)            
    ...
    integer :: idxa1
    real (kind=rk) :: temp(numPointsA, numAIME, numPointsSPA, numPointsY,numPointsL,numPointsA), tempEV(numPointsA, numAIME, numPointsSPA, numPointsY)
    character(len=1024) :: outFile
    ...
    do s = 1,numsims,1
        ...
        do t = 1,tperiods,1                              ! loop through time periods for a particular individual
            !write (outFile, *), trim("..\\PolicyFuncs\polType"),typeSim,trim("Period"),t,".txt"
            !outfile=ADJUSTL(outfile)
            !open (unit=201, file=outfile, status='unknown', action='read')
            !read (201,*) temp
            !close( unit=201)
            !
            !temp = temp - modelObjects%policy(t,typeSim,:,:, :,:,:,:) 
            !write (*,*) maxval(temp)
            !
            !write (outFile, *), trim("..\\PolicyFuncs\EVType"),typeSim,trim("Period"),t,".txt"
            !outfile=ADJUSTL(outfile)
            !open (unit=201, file=outfile, status='unknown', action='read')
            !read (201,*) tempEV
            !close( unit=201)
            !
            !tempEV = tempEV - modelObjects%EV(t,typeSim,:,:, :,:) 
            !write (*,*) maxval(temp)             
