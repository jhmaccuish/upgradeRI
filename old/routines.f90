...
#include "globalMacros.txt"
    ...
    do typeSim = 1, numPointsType
        ...
        do t=1 , Tperiods
            ...
            if (t <=params%ERA(typeSim)) then
                a = ((upper(typeSim,t+1)-500.0)/t)/(numAIME-1) !-3700

                grids%AIMEgrid(typeSim,t,:) = a*(/(i,i=0,numAIME-1)/)
                grids%AIMEgrid(typeSim,t,:) = grids%AIMEgrid(typeSim,t,:) + 500.0! +3700
            else
    ...
    integer :: ixl,  ixA1, i, j, dim, dimD ,K, labourChoices, offset, testInt, maxmaxA, sizeBuffer !A1
    ...
    integer, save :: saveDim(2,numPointsL), lastixT = 0, unemp, div, locl, locA1, morethan(11) = 0 ,choices = 0
    ...
    real(kind=rk) :: diff, lowestUtil, psum
    !integer :: iter, D
    REAL(kind=rk) :: error
    LOGICAL :: logcheck
    ...
    if (sizebuffer> 1) then
        ...
        f = 0
        do j =1,  grids%supportSPA(ixt)
            f(1) = f(1) - grids%posteriorSPA(ixt,numPointsSPA-grids%supportSPA(ixt)+j)*log(denom(j))
        end do
        g(1) =  1.0- sum(x(1:n))
        ...
        do i=1, n
            do j =1,  grids%supportSPA(ixt)
                df(i)   = df(i)-((grids%posteriorSPA(ixt,numPointsSPA-grids%supportSPA(ixt)+j)*payoff(i,j))/(denom(j)))
            end do
        end do
        dg =  -1
        ifail = -1
