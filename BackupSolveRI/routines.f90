    ...
    integer :: numTasks, tasksPerCore, leftOverTasks, thisCoreStart, thisCoreEnd, requiredl, spa
                        ...
                        if (ixt < stopwrok) then
                            ...
                            do ixY = 1, numPointsY               ! points on income grid
                                ...
                                if (ixt >= TendRI) then
                                    ...
                                    modelObjects%policyA1(ixt,typeSim,ixA,ixAIME, :,ixY,ixA1) = 1.0
                                ...
                                elseif (ixt == TendRI-1) then
                                    ...
                                    modelObjects%policyA1(ixt,typeSim,ixA,ixAIME, numPointsSPA,ixY,ixA1) = 1.0
                                    ...
                                    modelObjects%policyA1(ixt,typeSim,ixA,ixAIME, (/(spa,spa=1,numPointsSPA-1)/),ixY,ixA1) = 1.0
                                    ...
                                    call solvePeriodRI(params, grids, ixy, ixA, ixAIME ,ixt,typeSim, TendRI, EV1SPA, modelObjects)
                                    ...
                                    !    !do ixL = 0,(numPointsL-1),1
                                    !    !    do ixA1 = 1, numPointsA
                                    !    !
                                    !    !        !A    = grids%Agrid(typeSim,ixt, ixA1)
                                    !    !        !! THIS IS WRONG SHOULD BE TOMRROWS AIME
                                    !    !        !EV1SPA  = modelObjects%EV(ixt + 1,typeSim,ixa1,ixAIME,:,ixY)
                                    !    !    end do
                                    !    !end do
                                    !end do
                                    if (ixt>=Tretire) then
                                        ...
                                        EV1  = modelObjects%EV(ixt + 1,typeSim,:,:,ixt,ixY)! relevant section of EV matrix (in assets tomorrow)
                                        call solvePeriodRE(params, grids, ixy, ixA, ixAIME ,ixt,typeSim, ixt, EV1, ixA1,ixl,Va)
                                        modelObjects%policyA1(ixt,typeSim,ixA,ixAIME, (/(spa,spa=1,ixt-Tretire+1)/),ixY,ixA1) = 1.0
                            ...
                            modelObjects%policyA1(ixt,typeSim,ixA,ixAIME,:,:,ixA1)=1.0
    ...
    subroutine RISub(x,fvec)
    use header
    IMPLICIT NONE
    REAL(kind=rk), DIMENSION(:), INTENT(INout) :: x
    REAL(kind=rk), DIMENSION(:), INTENT(out) :: fvec
    !REAL(kind=rk), DIMENSION(:,:), INTENT(out) :: fjac
    call RIProblem(x,fvec,params, grids,grids%Ygrid(typeSim,ixt, ixY),grids%Agrid(typeSim,ixt, ixA), grids%AIMEgrid(typeSim,ixt,ixAIME), ixL,A1,EV1,ixt)
    END subroutine RISub
    end subroutine
    ...
    !LocpolicyA1(:,:,:,:,:,:,1) =  modelObjects%policyA1(:,:,:,:,:,(/(i,i=1,numPointsY-1,2)/))
    !LocpolicyA1(:,:,:,:,:,:,2) =  modelObjects%policyA1(:,:,:,:,:,(/(i,i=2,numPointsY,2)/))
    !LocpolicyL(:,:,:,:,:,:,1) =  modelObjects%policyL(:,:,:,:,:,(/(i,i=1,numPointsY-1,2)/))
    !LocpolicyL(:,:,:,:,:,:,2) =  modelObjects%policyL(:,:,:,:,:,(/(i,i=2,numPointsY,2)/))
    !LocEV(:,:,:,:,:,:,1) =  modelObjects%EV(:,:,:,:,:,(/(i,i=1,numPointsY-1,2)/))
    !LocEV(:,:,:,:,:,:,2) =  modelObjects%EV(:,:,:,:,:,(/(i,i=2,numPointsY,2)/))
    ...
    !! when RI is imporant
    ! ---------------------------------------------------------------------------------------------------------!
    subroutine solvePeriodRI(params, grids, ixy, ixA, ixAIME ,ixt,ixType,spa,  EV1, modelObjects)
    implicit none
    ...
    allocate(values(dim+1),parameters(dim+1,dim),ccp(dim))
    !cpp(:,:,:) = 0.0
    ...
    call amoeba(parameters,values, 0.0001_rk,func,iter)
    ...
    !        check = func(cpp(:,ixL+1,ixA1))
    !        !write(*,*), check
    !        print '("ixl = ",f6.3, "ixlA1= ",f6.3, " Value ",f16.3)',ixl,ixa1,check
    !    end do
    !end do
    ...
    real(kind=rk) :: func
    !local
    real(kind=rk) :: systemEq(dim), GrossUtil(dimD)!,grids%supportSPA(ixt)
    ...
    do d=1,dimD
        do y=1,grids%supportSPA(ixt)
            ...
            systemEq((d-1)*grids%supportSPA(ixt)+y)= p((d-1)*grids%supportSPA(ixt)+y)-exp(GrossUtil(d)) / sum(GrossUtil(:))
        end do
    end do
    !return MSE
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    subroutine  RIProblem(x,fvec,params, grids,Y,A0, AIME,L,A1,EV,ixt)
    use header
    IMPLICIT NONE

    type (structparamstype), intent(in) :: params
    type (gridsType), intent(in) :: grids
    real(kind=rk), intent(in) :: Y,A0, AIME, A1, EV(:,:)
    integer, intent(in) :: L, ixt
    REAL(kind=rk), DIMENSION(:), INTENT(inout) :: x
    REAL(kind=rk), DIMENSION(:), INTENT(out) :: fvec
    !REAL(kind=rk), DIMENSION(:,:), INTENT(out) :: fjac
