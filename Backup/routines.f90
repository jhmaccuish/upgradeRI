...
#include "globalMacros.txt"
    ...
    do typeSim = 1, numPointsType
        ...
        do ixt=Tperiods,1, -1                               ! Loop from time T-1 to 1
            ...
            ! numPointsY,numPointsL,numPointsA
            ...
            !read (201,format_numpeepcols_int) temp
            !close( unit=201)
            !
            !temp = temp - modelObjects%policy
            !write (*,*) maxval(temp)
            ...
            !open (unit=201, file=outfile, status='unknown',recl=requiredl, action='read')
            !read (201,format_numpeepcols_int) tempEV
            !close( unit=201)
            !
            !tempEV = tempEV - modelObjects%EV
            !write (*,*) maxval(temp)
    ...
    do s = 1,numsims,1
        ...
        do t = 1,tperiods,1                              ! loop through time periods for a particular individual
            ...
            do i=1,numPointsL
                do j=1,numPointsA
                    policy(:,:,j+(i-1)*numPointsA) = LocmodelObjects(unemployed)%policy(:,:,TrueSPA,idxy(1),i,j)
                end do
            end do
            ccp =  reshape(LocmodelObjects(unemployed)%policy(a(1, s),idxaime(1),TrueSPA,idxy(1),:,:),(/numPointsL*numPointsA/))
            check1 = abs(sum(ccp))
            y(t, s) = yex(t,s)
            ...
            if (abs(sum(ccp - ccpTemp)) > 0.00001) then
                ...
                idxa1= (i-1)/2+1
                a(t+1, s) = idxa1
                mat = LocmodelObjects(unemployed)%policy(a(1, s),idxaime(1),TrueSPA,idxy(1),:,:)
