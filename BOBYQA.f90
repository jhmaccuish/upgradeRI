    ...
    subroutine bobyqa (n, npt, x, xl, xu, rhobeg, rhoend, iprint, maxfun, calfun, saveSt, reload)
    ...
    !! recommended.
    real(rk),dimension(:),intent(inout) :: x       !! Initial values of the variables must be set in X(1),X(2),...,X(N). They
    !! will be changed to the values that give the least calculated F.
    real(rk),dimension(:),intent(in)    :: xl      !! lower bounds on x. The construction of quadratic models
    !! requires XL(I) to be strictly less than XU(I) for each I. Further,
    !! the contribution to a model from changes to the I-th variable is
    !! damaged severely by rounding errors if XU(I)-XL(I) is too small.
    real(rk),dimension(:),intent(in)    :: xu      !! upper bounds on x. The construction of quadratic models
    !! requires XL(I) to be strictly less than XU(I) for each I. Further,
    !! the contribution to a model from changes to the I-th variable is
    !! damaged severely by rounding errors if XU(I)-XL(I) is too small.
    real(rk),intent(in)                 :: rhobeg  !! RHOBEG must be set to the initial value of a trust region radius.
    ...
    real(rk),intent(in)                 :: rhoend  !! RHOEND must be set to the final value of a trust
    ...
    !save the state to allow reloading  and/or reload from previously saved verions
    logical, intent(in) :: reload, saveSt
    ...
    call bobyqb (n, npt, x, xl, xu, rhobeg, rhoend, iprint, maxfun, w(ixb), w(ixp), &
        ...
        w(isu), w(ixn), w(ixa), w(id), w(ivl), w(iw), calfun, saveSt, reload)
    ...
    real (kind=rk) :: x (*), xl (*), xu (*), xbase(*), xpt (npt,*), fval(*), xopt (*), &
        gopt (*), hq (*), pq (*), bmat (ndim,*), zmat (npt,*), sl (*), su (*), &
        xnew (*), xalt (*), d (*), vlag (*), w (*)
    real (kind=rk) :: rhobeg, rhoend, xoptsq, fsave, rho, diffa, diffb, delta, temp, crvmin, dsq, dnorm, &
        distsq, diffc, errbig, frhosq, bdtol, bdtest, curv, fracsq, sumpq, sumz, sumw, adelt, &
        cauchy, alpha, suma, sumb, beta, bsum, dx, denom, delsq, biglsq, hdiag, den, f, fopt, &
    ...
    integer :: iprint, maxfun, N, np, npt, nptm, nh, kopt, nf, i, kbase, nresc, ntrits, itest, j, k, ih, nfsav, &
        ip, jj, knew, jp, ndim, ksav


    logical :: saveSt, reload
    real (kind=rk), parameter :: half = 0.5_rk
    real (kind=rk), parameter ::  one = 1.0_rk
    real (kind=rk), parameter :: ten = 10.0_rk
...
430         format (/ 4 x, 'Return from BOBYQA because a trust',&
        ...
        else if (ratio <= 0.7_rk) then
            delta = max (half*delta, dnorm)
        else
            delta = max (half*delta, dnorm+dnorm)
        end if
        if (delta <= 1.5_rk*rho) delta = rho
        !
...
680 if (rho > rhoend) then
        delta = half * rho
        ratio = rho / rhoend
        if (ratio <= 16.0_rk) then
            rho = rhoend
        else if (ratio <= 250.0_rk) then
...
700         format (/ 4 x, 'New RHO =', 1 pd11.4, 5 x, 'Number of',&
                ' function values =', i6)
            if (rank==0) print 710, fval (kopt), (xbase(i)+xopt(i), i=1, n)
...
70      format (/ 4 x, 'Function number', i6, '    F =', 1 pd18.10,&
    ...
    if (npt >= n+np) then
        do k = 2 * np, npt
            iw = (real(k-np, rk)-half) / real (n, rk)
            ip = k - np - iw * n
            iq = ip + iw
            if (iq > n) iq = iq - n
            ptsid (k) = real (ip, rk) + real (iq, rk) / real (np, rk) + sfrac
...
120 dsqmin = zero
    ...
    do k = 1, npt
        ...
        if (k == kopt) then
            ...
            iq = real (np, rk) * ptsid (k) - real (ip*np, rk)
            if (iq > 0) then
                iw = 1
                if (ip == 0) iw = 2
    ...
    if (denom <= 1.0e-2_rk*vlmxsq) then
...
260 do kpt = 1, npt
        ...
        pq (kpt) = zero
        ip = ptsid (kpt)
        iq = real (np, rk) * ptsid (kpt) - real (ip*np, rk)
        if (ip > 0) then
            xp = ptsaux (1, ip)
            xpt (kpt, ip) = xp
        end if
...
30  stepsq = zero
    ...
    if (gredsq*delsq <= 1.0e-4_rk*qred*qred) go to 190
    !
...
50  resid = delsq
    ...
    if (stplen < blen) then
        ...
        if (sdec <= 0.01_rk*qred) go to 190
        beta = gredsq / ggsav
        go to 30
    end if
90  crvmin = zero
...
120 iterc = iterc + 1
    ...
    if (temp <= 1.0e-4_rk*qred*qred) go to 190
    temp = sqrt (temp)
    do i = 1, n
        if (xbdi(i) == zero) then
...
150 shs = zero
    ...
    isav = 0
    redsav = zero
    iu = 17.0_rk * angbd + 3.1_rk
    do i = 1, iu
        angt = angbd * real (i, rk) / real (iu, rk)
    ...
    if (isav < iu) then
        ...
        angt = angbd * (real(isav, rk)+half*temp) / real (iu, rk)
    end if
    ...
    !
    if (sdec > 0.01_rk*qred) go to 120
190 dsq = zero
    do i = 1, n
...
210 ih = 0
    ...
    subroutine saveState(path, x, xl, xu, xbase, xpt, fval, xopt, gopt, hq, pq, bmat, zmat, sl, su, &
        xnew, xalt, d, vlag, w, rhobeg, rhoend, xoptsq, fsave, rho, diffa, diffb, &
        delta, temp, crvmin, dsq, dnorm, distsq, diffc, errbig, frhosq, bdtol, &
        bdtest, curv, fracsq, sumpq, sumz, sumw, adelt, cauchy, alpha, suma, sumb, &
        beta, bsum, dx, denom, delsq, biglsq, hdiag, den, f, fopt,  vquad, ratio, densav, &
    ...
    open (unit = 666, form="unformatted", file=trim(path) // 'hq', status='replace', ACCESS="STREAM", action='write')
    ilocal = iTotal - ihq
    write (666) hq(1:ilocal)
    close (unit=666)

    open (unit = 666, form="unformatted", file=trim(path) // 'pq', status='replace', ACCESS="STREAM", action='write')
    ilocal = iTotal - ipq
    ...
    open (unit = 666, form="unformatted", file=trim(path) // 'rhoend', status='replace', ACCESS="STREAM", action='write')
    write (666) rhoend
    close (unit=666)

    open (unit = 666, form="unformatted", file=trim(path) // 'xoptsq', status='replace', ACCESS="STREAM", action='write')
    write (666) xoptsq
    close (unit=666)

    open (unit = 666, form="unformatted", file=trim(path) // 'fsave', status='replace', ACCESS="STREAM", action='write')
    write (666) fsave
    close (unit=666)

    open (unit = 666, form="unformatted", file=trim(path) // 'rho', status='replace', ACCESS="STREAM", action='write')
    write (666) rho
    close (unit=666)

    open (unit = 666, form="unformatted", file=trim(path) // 'diffa', status='replace', ACCESS="STREAM", action='write')
    write (666) diffa
    close (unit=666)
    ...
    subroutine loadState(path, x, xl, xu, xbase, xpt, fval, xopt, gopt, hq, pq, bmat, zmat, sl, su, &
        xnew, xalt, d, vlag, w, rhobeg, rhoend, xoptsq, fsave, rho, diffa, diffb, &
        delta, temp, crvmin, dsq, dnorm, distsq, diffc, errbig, frhosq, bdtol, &
        bdtest, curv, fracsq, sumpq, sumz, sumw, adelt, cauchy, alpha, suma, sumb, &
        beta, bsum, dx, denom, delsq, biglsq, hdiag, den, f, fopt,  vquad, ratio, densav, &
    ...
    real(rk),dimension(100) :: x, xl, xu
    integer :: i,j,n,jcase,npt, m
    real(rk) :: temp

    real(rk),parameter :: twopi  = 8.0_rk * atan (1.0_rk)
    real(rk),parameter :: bdl    = - 1.0_rk
    real(rk),parameter :: bdu    = 1.0_rk
    integer,parameter  :: iprint = 2
    integer,parameter  :: maxfun = 500000
    real(rk),parameter :: rhobeg = 1.0e-1_rk
    real(rk),parameter :: rhoend = 1.0e-6_rk
    ...
    !                print 30, n, npt
    !30              format (/ / 5 x, ',  N =', i4, '  and  NPT =', i4)
    !                !do j = 1, m
    !                !    temp = real (j, rk) * twopi / real (m, rk)
    !                !    x (2*j-1) = cos (temp)
    !                !    x (2*j) = sin (temp)
    !                !end do
    !                x = 1.0
...
30          format (/ / 5 x, '2D output with M =', i4, ',  N =', i4, '  and  NPT =', i4)
            ...
            call bobyqa (n, npt, x, xl, xu, rhobeg, rhoend, iprint, maxfun, calfun, .TRUE., .TRUE.)
        end do
        m = m + m
        if (m > 5) exit !10
    end do
    ...
    do i = 4, n, 2
        do j = 2, i - 2, 2
            ...
            temp = max (temp, 1.0e-6_rk)
            f = f + 1.0_rk / sqrt (temp)
        end do
    end do
